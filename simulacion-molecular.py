...
# ------------------------------------- CONTENIDO 3D Y GRAFICO -----------------------------------------       
    ...
    with col2:
        ...
        st.markdown(titulo, unsafe_allow_html=True)
        if st.session_state.selected_option == "Un rango de distancias":
            titulo2 = f'<p style="color: #FFFFFF; padding: 3px; margin-left: 50px;">Energía de la molécula en función de la geometría</p>'
            st.markdown(titulo2, unsafe_allow_html=True)
        else:
            titulo2 = f'<p style="color: #FFFFFF; padding: 3px; margin-left: 50px;">Convergencia del optimizador del VQE</p>'
            st.markdown(titulo2, unsafe_allow_html=True)
        ...
        if st.session_state.selected_option == "Un rango de distancias" and len(st.session_state.resultado[1]) > 1:
            if st.session_state.selected_range is not None:
                ...
                if len(distancias) == len(energias):
                    ...
                    titulo = f'<hp style="color: #ffffff;margin-left: 120px; "><strong>⚪ Energía mínima: {min_energia} Å</h1>'
                    st.markdown(titulo, unsafe_allow_html=True)
                else:
