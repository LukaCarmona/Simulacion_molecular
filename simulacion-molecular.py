...
with st.sidebar:
    ...
    style = """
        <style>
            .container {
                border: 2px solid gray;  /* Estilo del borde */
    ...
    # Insertar el estilo al principio de la aplicación
    st.markdown(style, unsafe_allow_html=True)

    # Crear los contenedores con borde aplicando la clase "container"
    with st.container() as container1:
        ...
        # Ejemplo de mostrar información adicional
        if molecula == "LiH":
        ...
        # Mostrar información de la molécula seleccionada
    ...
    with st.container() as container2:
        ...
        with col1:
            ...
            if option == "Un Rango":
                ...
                new_distancias = [round(min(distancias) + i * step, 2) for i in range(int((max(distancias) - min(distancias)) / step) + 1)]
                range_values = st.select_slider("**Selecciona un rango de distancias**", options=new_distancias, value=(new_distancias[0], new_distancias[-1]))
            
            elif option == "Un Punto":
                distancia_min = st.number_input('**Especifique la distancia en la que quiere calcular**', min_value=min(distancias), max_value=max(distancias), value=min(distancias), step=0.3, format="%.1f")
